...
当前版本是 **3D 星球展厅**：打开网站先看到粉白能量大爆炸，随后进入一颗可旋转的星球。关于小白、精选、小红书、B站、关注分别位于星球的不同侧面；点击导航或星球上的发光节点，相机会绕星球飞到对应一面，也可以直接拖拽旋转星球。内容浮层不参与 3D 布局，因此不会互相遮挡。
...
支持搜索、日夜粉白主题切换、内置原创八音盒（默认关闭）。3D 背景为纯程序化生成（能量核心、星球网格、粒子爆发、冲击波、星云、bloom），不加载任何纹理图片。切换分区时会触发镜头飞行、闪光与星云配色渐变；鼠标移动有 3D 视差与光点拖尾，点击会溅出星尘，作品卡片支持 3D 倾斜和反光。按 B 可随时触发脉冲跃迁，按 / 可快速搜索。界面图标使用本地打包的 Lucide 线性 SVG 图标，不使用 emoji。
